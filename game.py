...
class Game:
    ...
    def __init__(self):
        ...
        self.build_menu(self.game_logic.get_game_board())
    ...
    def __refresh(self):
        while True:
            game_board = self.game_logic.get_game_board()
            self.build_board(game_board)
            time.sleep(0.1)


    def build_menu(self, game_board):
        ...
        self.play_button = tk.Button(master=self.button_frame, text='PLAY', command=self.build_board(game_board))
    ...
    def build_board(self, game_board):

        self.row_num = len(game_board)
        self.col_num = len(game_board[0])
